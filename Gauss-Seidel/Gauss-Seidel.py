...
def gauss_seidel(A, b, x0, tol=1e-6, maxiter=100):
    n = len(b)
    x = x0.copy()
    iteracoes = []
    erros = []
...
for caso in casos_teste:
    ...
    raiz, erro_final, iteracoes, erros, tabela = gauss_seidel(A, b, x0, margem)
    ...
    plt.yscale("log")
    plt.title(f"Caso {idx} - Erro por iteração (Gauss-Seidel)")
...
with open("saida.txt", "w", encoding="utf-8") as arq:
    arq.write("Resultados do Método de Gauss-Seidel\n\n")
